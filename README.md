...
Windows 桌面生态小游戏，v0.4.1 桌面交互修复版。C# + WPF，使用系统自带二维绘图，无 NuGet 依赖、无服务器、无需管理员权限。

v0.4.1 修复快捷菜单关闭时触发失焦、再次调用 Close 导致程序退出的问题。失焦改为延迟关闭，窗口关闭阶段防止重复关闭，重新获得焦点取消待处理的关闭请求。沿用原有存档路径。
...
桌面入侵后，悬浮工具栏可点击箭头收起/展开。收起会自动回到日常办公模式，仅保留展开、召唤桌宠和暂停隐藏入口。

点击悬浮栏「召唤桌宠」可就地查看六只桌宠的状态、体力与任务，选择后直接派去灭虫、清扫或休息，无需打开控制台。点击已派出桌宠的主体，可打开该桌宠的小菜单，调整任务、喂食、照顾或召回；点击外部或按 Esc 关闭菜单。分身不单独派遣，随主体调整。

日常办公时仅桌宠身体附近的小区域接收鼠标，其他覆盖层位置继续穿透。召回、全屏自动隐藏、停止入侵会一并隐藏或移除这些点击区域。多屏仍采用相同生态映射，混合 DPI 需更多实机验证。
...
程序运行占用 exe 时，可使用 `build.ps1 -OutputDirectory bin-v04` 或 `package.ps1 -OutputDirectory bin-v04` 独立构建。运行新版之前，请从旧版托盘退出，以释放单实例锁。
...
- `src/DesktopControls.cs`：桌面召唤列表、单宠任务菜单和跟随点击区域。

## Agent 方案

见 `docs/agent-plan.md`。建议用单个规划 Agent 做自然语言指挥、自动排班和角色对话，既有模拟负责移动与技能。通过只读快照及白名单命令接入，异步调用模型，断网继续本地游戏。本轮仅提供接入方案，未添加外部模型调用。
